...
namespace PPT.Pages
{
    ...
    public class BranchFinalReportModel : PageModel
    {
        ...
        public BranchFinalReportModel(UserManager<User> userManager, IWebHostEnvironment _environment, IRepository<Attendance> attendanceRepository)
        {
        ...
        public FileResult? OnGetDownloadFull(string monthF)
        {
            ...
            if (!DateTime.TryParse(monthF, out date))
                return null;
            List<AttendanceMapper> CSVList = _attendanceRepository.GetAttendanceByDateForBranchUndetailed(branch.ID, date, false, null);

            string fileName = branch.Name + "_FinalFullTime_" + "_" + date.Month + "_" + date.Year + ".csv";
        ...
        public FileResult? OnGetDownloadPart(string monthP)
        {
            ...
            if (!DateTime.TryParse(monthP, out date))
                return null;
            List<AttendanceMapper> CSVList = _attendanceRepository.GetAttendanceByDateForBranchUndetailed(branch.ID, date, true, true);

            string fileName = branch.Name + "_FinalPartTime_" + "_" + date.Month + "_" + date.Year + ".csv";
            string path = Path.Combine(this.Environment.WebRootPath, "MReports/") + fileName;
            using (var stream = System.IO.File.OpenWrite(path))
            using (var writer = new StreamWriter(stream, Encoding.UTF8))
            using (var csv = new CsvWriter(writer, CultureInfo.InvariantCulture))
